		...
		foreach ($objNodeArray as $objNode) {
			if (!($objNode instanceof Node\AbstractBase || $objNode instanceof iCondition)) {
				if (!$blnPreviousIsNode)
					throw new Caller('OrderBy clause parameters must all be Node\AbstractBase or iCondition objects followed by an optional true/false "Ascending Order" option', 3);
				$blnPreviousIsNode = false;
				...
				if (!$objNode->_ParentNode) {
					throw new InvalidCast('Unable to cast "' . $objNode->_Name . '" table to Column-based Node\AbstractBase', 4);
				}
